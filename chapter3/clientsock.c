...
int clientsock(const char* host, const char* service, const char* transport, struct sockaddr_in* fsin)
{
    ...
    if (type == SOCK_STREAM)
    {
        fprintf(stderr, "[CLIENT] connecting to service %s.. \n", service);
        if (connect(s, (struct sockaddr*)sin, sizeof(*sin)) != 0)
            errexit("can't connect to service: %s\n", strerror(errno));
        fprintf(stderr, "[CLIENT] service %s connected. \n", service);
    }
    if (fsin == NULL)
        free(sin);
    return s;
